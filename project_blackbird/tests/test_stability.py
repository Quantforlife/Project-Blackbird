...
def test_start_live_returns_explicit_block_reason() -> None:
    engine = SimulationEngine(rows=1, columns=2)
    controller = RealTimeController(simulation_engine=engine, event_bus=engine.event_bus, tick_seconds=0.01)
    socket_bridge = SocketServerBridge(event_bus=engine.event_bus, socketio=MockSocket())
    bridge = ControllerBridge(controller, socket_bridge)

    first = bridge.start_live()
    second = bridge.start_live()
    bridge.shutdown()
    engine.event_bus.shutdown()
...
def test_realtime_diagnostics_endpoint(client) -> None:
    resp = client.get('/realtime/diagnostics')
    assert resp.status_code == 200
    if hasattr(resp, "get_json"):
        payload = resp.get_json()
        assert "controller" in payload
        assert "socket_events" in payload
    else:
        assert resp.data == b"{}"
