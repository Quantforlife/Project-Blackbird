...
@realtime_bp.get("/diagnostics")
def diagnostics() -> Response:
    bridge = _bridge()
    data = bridge.diagnostics()
    data["socket_connected_clients"] = bridge.socket_bridge._client_count
    data["debug_diagnostics"] = current_app.config.get("DEBUG_DIAGNOSTICS", False)
    return jsonify(data)
...
def command(action: str) -> Response:
    bridge = _bridge()
    if action == "start":
        payload = bridge.start_live()
        status = 200 if payload.get("status") == "running" else 409
...
def mode(mode: str) -> Response:
    ...
    if mode == "live":
        return jsonify({"mode": "live", "status": "ready", "controller": bridge.controller.status()})
