...
def get_controller_bridge() -> ControllerBridge:
    global _runtime_bridge
    with _runtime_lock:
        if _runtime_bridge is None:
            simulation_engine = SimulationEngine()
            controller = RealTimeController(simulation_engine=simulation_engine, event_bus=simulation_engine.event_bus)
            socket_bridge = SocketServerBridge(event_bus=simulation_engine.event_bus)
            _runtime_bridge = ControllerBridge(controller, socket_bridge)
        return _runtime_bridge


def initialize_runtime(app) -> ControllerBridge:
    """Initialize singleton runtime services and bind to app."""
    bridge = get_controller_bridge()
